...
namespace UserInterfaceSample.Controls
{
    ...
    public class PageFlipTracker
    {
        ...
        {
            // support keyboard
            if (input.IsNewKeyPress(Microsoft.Xna.Framework.Input.Keys.Right, null, out PlayerIndex index))
            {
                BeginFlip(1);
            }
            else if (input.IsNewKeyPress(Microsoft.Xna.Framework.Input.Keys.Left, null, out PlayerIndex index2))
            {
                BeginFlip(-1);
            }
